...
public struct Configuration: Codable {
    ...
    public init(from decoder: Decoder) throws {
        do {
            ...
            certificate = try .init(
                derEncoded: container.decode(Data.self, forKey: .certificate).bytes)
            privateKey = try .init(
                P256.Signing.PrivateKey(
                    derRepresentation: container.decode(Data.self, forKey: .privateKey)))
        } catch {
    ...
    var context: NIOSSLContext {
        // swiftlint:disable:next force_try
        try! .init(
            configuration: try TLSConfiguration.makeServerConfiguration(
                certificateChain: [
